...
class BaseCovariateExperiment(pl.LightningModule):
    ...
    def prepare_data(self):
        ...
        for i in range(self.n_prior_flows):
            setattr(self.pyro_model, f'prior_flow_permutation_{i}', torch.randperm(self.latent_dim, dtype=torch.long, requires_grad=False).to(self.torch_device))
            setattr(self.pyro_model, f'posterior_flow_permutation_{i}', torch.randperm(self.latent_dim, dtype=torch.long, requires_grad=False).to(self.torch_device))

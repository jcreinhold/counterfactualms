...
from counterfactualms.utils import linalg as LA
...
class Wishart(TorchDistribution, ExponentialFamily):
    ...
    @lazy_property
    def scale_tril(self):
        return LA.inverse_cholesky(2. * self.scale)
    ...
    def _log_normalizer(self, eta1, eta2):
        D = self.event_shape[-1]
        a = -eta1 - .5 * (D + 1)
        return torch.mvlgamma(a, D) - a * LA.posdef_logdet(-eta2)[0]
    ...
    def log_prob(self, value):
        ...
        D = self.event_shape[-1]
        logdet = LA.posdef_logdet(value)[0]
